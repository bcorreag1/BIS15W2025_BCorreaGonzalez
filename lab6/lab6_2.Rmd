...
msleep %>% 
  ...
  summarize(across(everything(), mean, na.rm=TRUE)) #if it says that across is depricated (old)
```
Or could do
...
msleep %>% 
  filter(bodywt > 100) %>% 
  summarize(n_genera=n_distinct(genus)) # number of distinct/unique observations in a categorical variable
```
But what if you want the genus names? 
way 1:
```{r}
bodywt_sm <- msleep %>% 
  filter(bodywt > 100)
...
```{r}
msleep %>% 
  count(vore) #table does not give the amount of NA
```

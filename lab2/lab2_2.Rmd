...
## Load the tidyverse
```{r}
library("tidyverse") #load this every time for it to work
```
...
The first row.  
```{r}
hbirds[1,] #whats in the first row
```

The third column.  
```{r}
hbirds[ ,3] #whats in the third column
```

## Calculations
We can use the `$` to access a column (variable) in a data frame. Here we calculate the mean length of the hummingbirds.  
```{r}
mean(hbirds$length) #finding the mean of the length column
```

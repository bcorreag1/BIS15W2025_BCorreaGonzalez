...
```{r}
filter(fish, lakeid == "AL") #searching for all of the rows with "AL" in lakeid. Make sure to use TWO EQUAL SIGNS.
```
...
```{r}
filter(fish, lakeid != "AL") #dont need to put the variable name in quotes but you do for objects? or what you are searching for. Using ! excludes all the operations that ARE NOT "AL"
factor(fish$lakeid)
```
...
```
```{r}
filter(fish, length==167)# cant fnd the next one so use the %in% operator.
```
...
```{r}
filter(fish, near(radii_length_mm, 2, tol = 0.2)) #tolerance is the range you are allowing for it to be near.
```
...
```{r}
filter(fish, lakeid !="AR") #!= --> not equal to
```

3. Filter the fish data to include all fish with a scalelength within 0.25 of 8.
```{r}
filter(fish, near(scalelength, 8, tol = 0.25)) #tol = tolerance level
```
...
```{r}
filter(fish, scalelength %in% c(2,4)) #doesnt work because theres are no values that are exactly 2 or 4. 
```
```{r}
filter(fish, scalelength==2) #see theres nothing
```

...
```{r}
# uses a more organizaional philosphy. this is distint from the base R code that people use. Base R code is different although it works similarly. but here we will mostly be using tidyverse and add on some little bits of base R.
```
...
```{r}
#one of the commands allows you to pull out specific data ranges.
```
...
```{r}
# select is pulling out variables of interest/
```
...
```{r}
select(fish, "lakeid", "scalelength") #(1,2,3) 1= data set. 2&3 are selecting the variables/columns
# can be used to focus attention for yourself and stakeholders who may not be as experienced with R.
```

Remember that you can save the output to a new dataframe.
```{r}
fish_subset <- select(fish, "lakeid", "scalelength") #saving the selection as a new data table.
```

To add a range of columns use `start_col:end_col`.
```{r}
select(fish, fish_id:length) #first column you want : last column you want
```

The - operator is useful in select. It allows us to select everything except the specified variables.
```{r}
select(fish, -"fish_id", -"annnumber", -"length", -"radii_length_mm")
# the negative / not operator. saying "pull out" from fish dataset all of these variables. (leaves behind the ones that are not listed)
```
...
```{r}
select_if(fish, is.numeric) #only pulling out the variables that are numeric, variables are columns
```

To select all columns that are *not* a class of data, you need to add a `~`.
```{r}
select_if(fish, ~!is.numeric(.)) #the ! means not
# its pulling out everything that is not numeric.
```
...
```{r}
mammals_max.life <- select(mammals, "max. life")
select(mammals_max.life, contains("NA"))
mean(mammals$"max. life")
anyNA(mammals) #best and easiest code to use
```
It appears as though there are no NA's, but there are weird values.
...
```{r}
mammals_new <- rename(mammals, genus = "Genus", wean_meass = "wean mass" , max_life= "max. life", litter_size = "litter size", litters_per_year = "litters/year") #only want to run once
```

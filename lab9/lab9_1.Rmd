...
No, not tidy. The column names are variables.
```{r}
expenditure_tidy <- expenditures %>% 
  pivot_longer(-expenditure, #not moving expenditure column
               names_to = "year",
               values_to = "dollars")
expenditure_tidy
```
...
```{r}
heartrate2 %>% 
  separate(patient, into= c("patient", "sex"), sep = "_") %>% #not always the case, some data wont have the underscore
...
```{r}
heartrate2 %>% 
  separate(patient, into= c("patient", "sex"), sep = -1) # removes the last character in the string, but underscore remains.
```
...
heartrate2 %>% 
  ...
  mutate(patient=str_remove(patient, "_")) %>%  #strngr package helps deal with strings. "remove the underscore from patients"
    pivot_longer(-c(patient,sex),
               names_to = "drug",
               values_to = "heartrate")
```
...
```{r}
heartrate3 %>% 
  unite(paient_sex, "patient", "sex", sep="_")
```
...
```{r}
beachbugs <- read.csv("data/beachbugs_long.csv") %>% clean_names()
beachbugs
```

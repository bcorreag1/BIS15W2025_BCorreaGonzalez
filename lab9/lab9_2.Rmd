...
In short, **plot= data + geom_ + aesthetics**.  geom type = plot type (histogram, dot plot etc)
...
glimpse(iris)
```
GROUP PROJECT HAS TO HAVE CATGECORICAL AND CONTINOUS DATA. 
...
```{r}
ggplot(data=iris, #indicates data
       mapping = aes(x=Species, y=Petal.Length)) #maps the aesthetics
```
...
```{r}
ggplot(data=iris, #indicates data
       mapping = aes(x=Species, y=Petal.Length)) +   #need a plus not pipe
  geom_boxplot()
```
...
ggplot(data=iris, #indicates data
       mapping = aes(x=Species, y=Petal.Length)) +   #need a plus not pipe
  geom_point() # THIS IS WRONG> CANT USE POINT WITH A CATEGORICAL DATA
```
...
ggplot(homerange,
       mapping = aes(x= log10.mass, y = log10.hra)) +
  geom_point() + 
  geom_smooth(method = lm, se=T) #lm = linear model, se = standard error. If you put F for se then grey will be removed. 
```

### Practice
1. What is the relationship between log10.hra and log10.preymass? What do you notice about how ggplot treats NA's?

R just doesnt plot NA's. 
...
ggplot(homerange,
  ...
  geom_point(na.rm=T) + 
  geom_smooth(method = lm, se=T, na.rm=T) #have to deal with the NAs within the functon
```
...
```{r}
homerange %>% 
  count(trophic.guild) #give a table first before plotting data
```
...
homerange %>% 
  ...
  geom_bar() #use this when making counts ofa categorical data. But what if you want to specify an X and a Y?
```
...
homerange %>% 
  ...
  geom_col() + 
  coord_flip() # transforms the data so now horizontal instead of vertical
```
...
homerange %>% 
  ...
  select(common.name, log10.mass) %>% 
  ggplot(aes(x = common.name, y = log10.mass)) + 
  geom_bar(stat="identity") #if you dont add the stat identity then will give error. 
```

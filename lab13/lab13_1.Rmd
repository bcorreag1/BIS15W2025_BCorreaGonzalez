...
```{r}
q <- homerange %>% 
  select(log10.mass, log10.preymass, taxon) %>% 
...
```{r}
p+theme_linedraw()+
  theme(legend.position = "bottom", #adjusts position of the legend; could also use "top"
...
+`scale_fill_brewer()` is for fills / barplots
...
```{r}
q + scale_colour_brewer(palette = "Dark2")
```
```{r}
...
```{r}
my_palette <- paletteer_d("colorBlindness::Blue2DarkRed18Steps")
```
...
```{r}
my_palette2 <- paletteer_d("MoMAColors::Picabia")
barplot(rep(1,6), axes=FALSE, col=my_palette2)
q + scale_color_manual(values = paletteer_d("MoMAColors::Picabia"))
```
...
```{r}
homerange %>% 
  select(log10.mass, thermoregulation) %>%
...
```{r}
homerange %>% 
  select(log10.mass, locomotion, thermoregulation) %>%
  ggplot(aes(x= log10.mass))+
  geom_density(fill = "darkblue", alpha = 0.6) +
  facet_grid(locomotion~.) #rows by columns. if want columns then put .~ in front. facet wrap only does it by rows/
```

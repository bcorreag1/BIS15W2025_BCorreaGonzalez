...
```{r}
inner_join(table_A, table_B, by="customer_ID") # werent on both tables so they were dropped. but when they are there it adds colmns to the end.
```

`left_join`
Where did customer 4 go? Notice the missing data for customer 3.
```{r}
left_join(table_A, table_B, by="customer_ID") #matching table b to table a. This is because 4 is not present in the left table (table_a).
```

`right_join`
Where did customer 3 go? Notice the missing data for customer 4.
```{r}
right_join(table_A, table_B, by="customer_ID") #matching table a to table b. This is because 3 is not present in the right table (table_b).
```

`full_join`
```{r}
full_join(table_A, table_B, by="customer_ID") #joins everythng as long as there are unique idnetifiers. in this case there is "customer_ID".
```

`anti_join`
Provides the rows in the first table for which there are not matching values in the second table.
```{r}
anti_join(table_A, table_B, by="customer_ID") #best for troubleshooting. more for finding obersvations that are being problematic.
```
...
```{r}
full_spider <- full_join(spider_records, spiders_locs, by = "Accession")
full_spider2 <- left_join(spider_records, spiders_locs, by = "Accession")
```
...
```{r}
write.csv(full_spider2, file = "full_spider2.csv.csv", row.names=F)
```

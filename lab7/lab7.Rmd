...
```{r}
penguins %>% 
  group_by(species) %>% #grouping by a CATEGORICAL Variable. Wont work bes twith a numerical variable
  summarize(mean_body_mass=mean(body_mass_g, na.rm=T),
            sd_body_mass=sd(body_mass_g, na.rm=T),
            n=n())
```
...
msleep %>% 
  group_by(order) %>% 
  summarize(brain_weight = mean(brainwt),
            n=n())
```
...
msleep %>% 
  group_by(order) %>% 
  summarize(mean(brainwt,na.rm=TRUE),n=n())
...
```
The scientists potentially didnt weigh these brains.
...
penguins %>% 
  group_by(species, island) %>%
  summarize(mean_bill_length_mm = mean(bill_length_mm), n=n(), .groups= 'keep')
```
```{r}
x <- penguins %>% 
  group_by(species, island) %>%
  summarize(mean_bill_length_mm = mean(bill_length_mm), n=n(), .groups= 'keep') # keeps all grouping
```
...
y <- penguins %>% 
  group_by(species, island) %>%
  summarize(mean_bill_length_mm = mean(bill_length_mm), n=n(), .groups= 'drop') # removes all grouping
```
...
z <- penguins %>% 
  group_by(species, island) %>%
  summarize(mean_bill_length_mm = mean(bill_length_mm), n=n(), .groups= 'drop_last') # drop last variable in the grouping. this is the DEFAULT
```
...
```
```{r}
# missing. something weirfd according to proff
```
...
```{r}
penguins %>%
  summarize(across(c(species, island, sex), n_distinct)) # old way and they are changing
```
```{r}
penguins %>% 
  summarize(across(c(species, island, sex), \(x) n_distinct(x, na.rm=T))) #te function takes x (each column that we specified) and applies n_distinct
```

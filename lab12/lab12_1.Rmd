...
deserts %>% 
  ...
  group_by(year, species_id) %>% 
  summarize(n=n(), .groups = 'keep') %>% 
  ggplot(aes(x=year, y=n, group=species_id, color=species_id))+ #group is what makes the line
  geom_line() +
  geom_point(shape=5)+ #can use different shapes
...
```{r}
homerange %>% 
  ggplot(aes(log10.mass))+ #dont have to include a y variable or directly express x
  geom_histogram(bins=20) #can control the way the distribution looks. 
```
...
homerange %>% 
  ...
  geom_histogram(color= "black", fill = "steelblue4") #the color is used for the outline and fill is for the fill. 
```

## Density plots  
Density plots are similar to histograms but they use a smoothing function to make the distribution more even and clean looking. They do not use bins. APPLIES A SMOOTHING FUNCTION.
```{r}
homerange %>% 
  ggplot(aes(log10.mass))+
  geom_density(fill="deepskyblue", color= "black", alpha=0.4) #alpha helps with transparency
```
...
homerange %>% 
  ...
  geom_histogram(fill = "deepskyblue4", alpha = 0.4, color = "black")+
...
```{r}
summary(homerange$log10.mass) #using baseR
```
...
table(quartiles) #giving range of each quartile
```

```{r}
homerange <- homerange %>% 
  mutate(mass_category = case_when(log10.mass<=1.7 ~ "small",
                                   log10.mass>1.7 & log10.mass<=3.3 ~ "medium",
...
```{r}
quartiles2 <- quantcut(homerange$log10.hra)
table(quartiles2)
```

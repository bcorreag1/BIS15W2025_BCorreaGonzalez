...
```{r}
filter(fish, lakeid=="AL" & length>350) #only pulls out the fish from AL with length greater than 350.
```

Notice that the `|` operator generates a different result. Why?
```{r}
filter(fish, lakeid=="AL" | length>350) # | is "or so now pulling out fishes from AL OR the ones with a length greater than 350. 
```
...
```{r}
filter(fish, length>400, (scalelength>11 | radii_length_mm>8)) #nesting scalelength into the bigger function
```
...
```{r}
mammals_Felidae <- filter(mammals, family == "Felidae")
mammals_Feliidae_gestation <- filter(mammals_Felidae, gestation < 3)
mammals_FG <- select(mammals_Feliidae_gestation, "genus", "family", "gestation")
mammals_FG2 <- filter(mammals_FG, gestation>-999)
```


```{r}
#Proffesor's way. I did the same thing except I didnt use the & to connect my first two lines of code

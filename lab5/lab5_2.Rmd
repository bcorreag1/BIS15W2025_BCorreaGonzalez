...
```{r}
fish %>% #specifies the dataframe
  select(lakeid, scalelength) %>% #piped into select
  filter(lakeid == "AL") #piped into filter
  # shift,command,m is the shortcut for %>% = "pipe"
#didnt need to tell select fish again because of the pipe
# ORDER MATTERS. The following pipe only uses the previous one. Use select before filter.
```

Here we select family, genus, and species then filter for gestation greater than 15 months.
```{r}
mammals %>% #specify the datafram
  select(family, genus, species, gestation) %>% #pipe into select
  filter(gestation>15) #pipe into filter
# now you have to use pipes for everything
```
...
```{r}
fish %>% #specify the fish dataset
  select(lakeid, radii_length_mm) %>% #piped into select to pull out lakeid and radii length
  filter(lakeid=="AL" | lakeid =="AR") %>% # filter to pull out the lakes. AND or Comma does not work
  filter(between(radii_length_mm, 2,4)) %>%  #extract between 2 and 4 radii length
...
fish %>% 
  select(lakeid, scalelength) %>% 
  arrange(desc(scalelength)) #descneding order with desc or use negative sign (-)-- look at practie question 1
```
...
fish %>% 
  ...
  filter(lakeid =="AL") %>% 
  arrange(fish_id) #sorts by fish id
```
...
mammals %>% 
  ...
  mutate(mass_kg=mass/1000) %>% #have to do this second because then you might have a -999*10
